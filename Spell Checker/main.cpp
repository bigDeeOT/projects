...
void fruityTest(void) {
	...
	char article[] = "(*&^%$#@#$%^&* $%^&*()(*&^ apple Dewayne Mor$#@#$%^&* $%^&*rison $#@#$%^&* $%^&*Perry &^%^&* orange *&^%^&";
...
void generalTest3(void) {
	printf("\n\n******* Starting General Test #2 *******\n");
	readFile("long-string-test.txt", article);
	readFile("test-dictionary.txt", dictionary);
	spellCheck(article, dictionary);
	printf("****DONE****\n");
}
...
int main(void) {
	...
	*/
	...
	int delta_t_ms = (int)std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time).count();
